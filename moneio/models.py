...
class Account(models.Model):
  user = models.ForeignKey(User, on_delete=models.CASCADE, related_name="account_owner")
...
class MoneyInOut(models.Model):
  user = models.ForeignKey(User, on_delete=models.CASCADE, related_name="money_in_out_owner")
  name = models.CharField(max_length=255)
  ...
  def __str__(self):
    return f"{self.user.username} -> {self.name}: {self.price}"
